...
impl<Lens> Store<EmailClientState, Lens> {
    pub(super) fn active_folder_label(&self) -> &'static str {
    ...
    pub(super) fn visible_message_ids(&self) -> Vec<String> {
        let active_folder = self.active_folder().cloned();
    ...
    pub(super) fn selected_message_uid(&self, visible_ids: &[String]) -> Option<String> {
    ...
    pub(super) fn selected_message_index(
        &self,
    ...
    pub(super) fn folder_count(&self, folder_id: FolderId) -> u32 {
        let messages_store = self.messages();
    ...
    pub(super) fn set_active_folder(&mut self, folder_id: FolderId) {
        self.active_folder().set(folder_id);
        self.close_read_pane();
    }

    pub(super) fn set_active_tab(&mut self, tab_id: TabId) {
    ...
    pub(super) fn set_selected_tags(&mut self, tags: Vec<MessageTag>) {
        self.selected_tags().set(tags);
        self.close_read_pane();
    }

    pub(super) fn select_message(&mut self, uid: String) {
        self.selected_id().set(uid);
        self.read_open().set(true);
    }

    pub(super) fn close_read_pane(&mut self) {
    ...
    pub(super) fn set_compose_open(&mut self, open: bool) {
    ...
    pub(super) fn set_compose_to(&mut self, value: String) {
        self.compose_to().set(value);
    }

    pub(super) fn set_compose_subject(&mut self, value: String) {
        self.compose_subject().set(value);
    }

    pub(super) fn set_compose_body(&mut self, value: String) {
        self.compose_body().set(value);
    }

    pub(super) fn archive_message(&mut self, uid: String) {
    ...
    pub(super) fn snooze_message(&mut self, uid: String) {
        self.update_message(uid.clone(), |message| {
    ...
    pub(super) fn delete_message(&mut self, uid: String) {
        self.update_message(uid.clone(), |message| {
    ...
    pub(super) fn toggle_message_flag(&mut self, uid: String) {
        self.update_message(uid, |message| {
            message.flagged = !message.flagged;
        });
    }

    pub(super) fn toggle_message_star(&mut self, uid: String) {
        self.update_message(uid, |message| {
            message.starred = !message.starred;
        });
    }

    pub(super) fn move_message_to_trash(&mut self, uid: String) {
        self.update_message(uid.clone(), |message| {
            message.folder_id = FolderId::Trash;
        });
        self.close_if_selected(&uid);
    ...
    pub(super) fn remove_message_tag(&mut self, uid: String, tag: MessageTag) {
        self.update_message(uid, |message| {
            message.tags.retain(|current| *current != tag);
        });
    }

    pub(super) fn set_message_tags(&mut self, uid: String, tags: Vec<MessageTag>) {
        self.update_message(uid, |message| {
            message.tags = tags;
        });
    }
    ...
    pub(super) fn close_if_selected(&mut self, uid: &str) {
